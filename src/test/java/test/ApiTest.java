...
public class ApiTest {
	...
	public void test() throws Throwable {
		...
		// Looking for story files (name pattern: '<nr>_<name>') describing the
		// simulations
		File[] list = new File("src/test/resources")
		...
		for (File file : list) {
			...
			if (url == null) {
				throw new IllegalArgumentException(String.format(
						"Resource %s not found.", fileName));
			}

			List<String> lines = readLines(url.openStream(),
			...
			for (String line : lines) {
				 ...
				 */
					...
					args.put("startClient", new Class<?>[] { Config.class,
							Shell.class });
					args.put("startProxy", new Class<?>[] { Config.class,
							Shell.class });
					args.put("startFileServer", new Class<?>[] { Config.class,
							Shell.class });

					Method method = factory.getClass().getMethod(instruction,
							args.get(instruction));
					if (method == null) {
						throw new IllegalArgumentException(String.format(
								"Method '%s' not found.", instruction));
					}
					...
					Object component = method.invoke(factory, new Config(
							terminal), shell);
				 ...
				 */
					...
					if (component == null) {
						throw new IllegalStateException(
								String.format(
										"Cannot find component '%s'. Please start it before using it.",
										parts[0]));
					}
